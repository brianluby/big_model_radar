...
export function buildResearchPriorityPrompt(
  ...
  const digestSections = (digests: RepoDigest[]) =>
    digests.map((d) => `## ${d.config.name} (github.com/${d.config.repo})\n${d.summary}`).join("\n\n---\n\n");
  ...
  const webSection = webResults
    .map((r) => {
      const items = r.newItems
        ...
        .map(
          (item) =>
            `- [${item.title || item.url}](${item.url}) | ${item.category} | ${item.lastmod.slice(0, 10)}`,
        )
        .join("\n");
  ...
  const trendingSection =
    [
      ...trendingData.trendingRepos
        ...
        .map(
          (r) =>
            `- [${r.fullName}](${r.url}) | ★${r.totalStars} | +${r.todayStars ?? 0} today | ${r.description ?? ""}`,
        ),
      ...trendingData.searchRepos
        .slice(0, 10)
        .map(
          (r) =>
            `- [${r.fullName}](${r.url}) | ★${r.stargazersCount} | query: ${r.searchQuery} | ${r.description ?? ""}`,
        ),
